...
export default function StatsCard({ title, value, icon, trend, subtitle, className = '' }: StatsCardProps) {
  return (
    <div className={`card hover:border-white/[0.12] transition-all ${className}`}>
...
function getIconBg(icon: string) {
  switch (icon) {
    case 'message': return 'bg-orange-500/20'
    case 'clock': return 'bg-green-500/20'
    case 'users': return 'bg-blue-500/20'
    case 'chart': return 'bg-orange-500/20'
